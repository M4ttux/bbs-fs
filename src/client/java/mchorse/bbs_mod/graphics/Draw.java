...
public class Draw
{
    ...
    private static final RenderPipeline POSITION_COLOR_OUTLINE = RenderPipelines.register(
        RenderPipeline.builder(RenderPipelines.POSITION_COLOR_SNIPPET)
            .withLocation(Identifier.of(BBSMod.MOD_ID, "pipeline/draw_outline"))
            .withVertexFormat(VertexFormats.POSITION_COLOR, VertexFormat.DrawMode.TRIANGLES)
            .withBlend(BLEND)
            .withDepthTestFunction(DepthTestFunction.LEQUAL_DEPTH_TEST)
    ...
    private static final RenderLayer OUTLINE_LAYER = RenderLayer.of(BBSMod.MOD_ID + "_draw_outline",
        RenderSetup.builder(POSITION_COLOR_OUTLINE).translucent().build());
...

    {
        ...
        renderBox(builder, stack, x, y, z, w, h, d, r, g, b, a, t);
        flush(builder, getPositionColorLayer());
    }

    /** Depth-tested outlines that leave the model depth untouched. */
    public static void renderBoxOutline(MatrixStack stack, double x, double y, double z, double w, double h, double d, float r, float g, float b, float a, float t)

    {
        BufferBuilder builder = Tessellator.getInstance().begin(VertexFormat.DrawMode.TRIANGLES, VertexFormats.POSITION_COLOR);
        renderBox(builder, stack, x, y, z, w, h, d, r, g, b, a, t);
        flush(builder, OUTLINE_LAYER);
    }
    ...
    {
        renderBox(builder, stack, x, y, z, w, h, d, r, g, b, a, 1 / 96F + (float) (Math.sqrt(w * w + h + h + d + d) / 2000));
    }

    public static void renderBox(BufferBuilder builder, MatrixStack stack, double x, double y, double z, double w, double h, double d, float r, float g, float b, float a, float t)
    {
        stack.push();
    ...
    {
        arc3D(builder, stack, axis, radius, thickness, r, g, b, startDeg, sweepDeg, segU, segV, a, 0F);
    }

    /** Arc with independent vertex opacity and tapered ends. */
    public static void arc3D(BufferBuilder builder, MatrixStack stack, Axis axis, float radius, float thickness, float r, float g, float b, float startDeg, float sweepDeg, int segU, int segV, float a, float taperDeg)
    {
        ...
        for (int iu = 0; iu < segU; iu++)
        {
            ...
            for (int iv = 0; iv < segV; iv++)
            {
                ...
                builder.vertex(mat, x11, y1 * width1, z11).color(r, g, b, a);
                builder.vertex(mat, x12, y2 * width1, z12).color(r, g, b, a);
                builder.vertex(mat, x22, y2 * width2, z22).color(r, g, b, a);

                builder.vertex(mat, x11, y1 * width1, z11).color(r, g, b, a);
                builder.vertex(mat, x22, y2 * width2, z22).color(r, g, b, a);
                builder.vertex(mat, x21, y1 * width2, z21).color(r, g, b, a);
            }
        }
