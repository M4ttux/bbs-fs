...
public class UIModelEditorRenderer extends UIFormRenderer implements GizmoViewport, UIBonePicker.Viewport
{
    ...
    private void renderOutline(MatrixStack stack, Model model, ModelNode node, int color)
    {
        ...
        Draw.renderBoxOutline(stack, min.x, min.y, min.z, max.x - min.x, max.y - min.y, max.z - min.z, Colors.getR(color), Colors.getG(color), Colors.getB(color), Colors.getA(color), OUTLINE_THICKNESS);
        stack.pop();
    }
    ...
    protected void update()
    {
        ...
        if (this.proceduralPreview >= 0)
        {
            ...
            this.entity.getLimbAnimator().updateLimbs(speed, 1F, 1F);
